    ...
    def process(self, batch: Any, detections: pd.DataFrame, metadatas: pd.DataFrame):
        ...
        for results, shape, (_, metadata) in zip(
            ...
            for bbox in results.boxes.cpu().numpy():
                ...
                if bbox.cls in [1, 2, 3] and bbox.conf >= self.cfg.min_confidence:
                    detections.append(
                        pd.Series(
                            dict(
                    ...
                    self.id += 1
                # check for "sports ball" class
                elif bbox.cls == 0 and bbox.conf >= self.cfg.min_confidence:
                    detections.append(
                        pd.Series(
                            dict(
                                image_id=metadata.name,
                                ...
                                bbox_conf=bbox.conf[0],
                                video_id=metadata.video_id,
                                category_id=0,  # "ball" class in posetrack
                            ),
                            name=self.id,
                        )
                    )
